...
.on('focusout','input',function(){
	     ...
	     $(function(){
        var _nava= $('#demo-list a');
        var _url = window.location.href;
        var _host = window.location.host;
        for(var i = 0; i<_nava.length ; i++){
            var _astr = _nava.eq(i).attr('href');
            if(_url.indexOf(_astr) != -1){
                _nava.eq(i).parent("#demo-list li").addClass('active').siblings().removeClass('active');
                _nava.eq(i).parents(".more").addClass('active').siblings().removeClass('active');
    ...
    $(document).ready(function() {
      $(".submenu-active").click(function () {
          $("li.more.in").removeClass('in')
