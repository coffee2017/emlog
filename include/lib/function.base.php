...
function pagination($count, $perlogs, $page, $url, $anchor = '') {
    ...
    for ($i = $page - 5; $i <= $page + 5 && $i <= $pnums; $i++) {
        if ($i > 0) {
            if ($i == $page) {
                $re .= "<span class='page-numbers current'>$i</span> ";
            } elseif ($i == 1) {
                $re .= "<a href=\"$urlHome$anchor\" class=\"page-numbers\">$i</a> ";
            } else {
                $re .= " <a href=\"$url$i$anchor\" class=\"page-numbers\">$i</a> ";
            }
        }
    }
    if ($page > 6)
...
function breakechoLog($content, $lid) {
    $ret = explode('[break1]', $content, 2);
    if (!empty($ret[1])) {
        $ret[0].='<p class="readmore"><a href="' . Url::log($lid) . '">阅读全文&gt;&gt;</a></p>';
        return $ret[0];
    } elseif(Option::get('isexcerpt') == 'y') {
